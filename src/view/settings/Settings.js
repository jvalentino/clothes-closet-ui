...
class Settings extends Component {
  ...
  async confirmDelete(settingsId) {
    const result = await controller.deleteSettings(
      settingsId,
      AppState.getSessionId(),
      AppState.getUrl()
    );
  ...
  async updateSetting(event) {
    ...
    const result = await controller.submitAdd(
      payload,
      AppState.getSessionId(),
      AppState.getUrl()
    );
      ...
      <div>
        ...
        <div className="standard-view">
          <div style={{ width: "100%", textAlign: "right" }}>
            <button className="default" onClick={AddSettingsModal.display}>
              Add New Setting
            </button>
          </div>
